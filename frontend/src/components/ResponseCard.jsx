...
const markdownComponents = {
  ...
  table: ({node, ...props}) => (
    <div className="my-2 overflow-x-auto">
      <table className="border-collapse text-[12px] w-full" style={{ border: '1px solid rgba(11,110,79,0.15)' }} {...props} />
    </div>
  ),
  thead: ({node, ...props}) => <thead style={{ background: 'rgba(11,110,79,0.06)' }} {...props} />,
  th: ({node, ...props}) => <th className="px-2 py-1.5 text-left font-semibold" style={{ border: '1px solid rgba(11,110,79,0.15)', color: 'var(--gold)' }} {...props} />,
  td: ({node, ...props}) => <td className="px-2 py-1.5" style={{ border: '1px solid rgba(11,110,79,0.10)' }} {...props} />,
        ...
        <div className="flex flex-wrap gap-2">
          {data.context.map((doc, i) => (
            <button key={i} onClick={() => onOpenSource?.(doc)}
              ...
              style={{ color: 'var(--gold-lo)', background: 'rgba(11,110,79,0.06)', border: '1px solid rgba(11,110,79,0.18)' }}>
      ...
      <div className="flex items-center gap-3 px-1 text-[10px]" style={{ color: 'var(--text-faint)' }}>
        {onOpenDetails && (
          <button onClick={() => onOpenDetails(data)} title="Details view — tools, LLM calls, retrieval trace"
            className="p-1 rounded-md transition-all hover:bg-[rgba(11,110,79,0.10)]"
